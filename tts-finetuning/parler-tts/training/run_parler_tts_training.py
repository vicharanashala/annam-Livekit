...
def main():
    ...
    if data_args.save_to_disk is not None and not dataset_was_precomputed:
        if accelerator.is_main_process:
            num_workers_to_use = data_args.preprocessing_num_workers if data_args.preprocessing_num_workers is not None else 1
            vectorized_datasets.save_to_disk(
                data_args.save_to_disk,
                num_proc=min(
                    num_workers_to_use, 
                    len(vectorized_datasets["eval"] if "eval" in vectorized_datasets else vectorized_datasets["train"]) - 1
                ),
            )
        accelerator.wait_for_everyone()
