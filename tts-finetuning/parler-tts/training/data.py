    ...
    **kwargs,
    ...
    for dataset_dict in tqdm(dataset_names_dict, desc="Combining datasets..."):
        with accelerator.local_main_process_first():
            if dataset_dict["name"].endswith(".csv"):
                features = datasets.Features({
                    "audio": datasets.Audio(sampling_rate=sampling_rate) if sampling_rate else datasets.Audio(),
                    "text": datasets.Value("string"),
                    "description": datasets.Value("string")
                })
                dataset = load_dataset(
                    "csv",
                    data_files={"train": dataset_dict["name"]} if dataset_dict["split"] == "train" else {dataset_dict["split"]: dataset_dict["name"]},
                    split=dataset_dict["split"],
                    features=features,
                    streaming=streaming,
                    **kwargs,
                )
            elif dataset_dict["name"].endswith(".json") or dataset_dict["name"].endswith(".jsonl"):
                dataset = load_dataset(
                    "json",
                    data_files={"train": dataset_dict["name"]} if dataset_dict["split"] == "train" else {dataset_dict["split"]: dataset_dict["name"]},
                    split=dataset_dict["split"],
            ...
            if sampling_rate is not None and audio_column_name is not None and not dataset_dict["name"].endswith(".csv"):
