...
async def create_speech(request: TTSRequest):
    ...
    try:
        ...
        # Reduce chunk size to 0.5s for smoother delivery (matching user snippet)
        frame_rate = model.audio_encoder.config.frame_rate
        play_steps = int(frame_rate * 0.5) 
        ...
        def audio_stream_generator():
            ...
            for new_audio in streamer:
                ...
                # Torchaudio Resampling REMOVED - Client handles 44.1kHz
                # new_audio is numpy array (float32), convert directly to Int16 PCM
                
                # ERROR FIX: new_audio is already numpy.ndarray
                audio_np = new_audio.squeeze()
                if isinstance(audio_np, torch.Tensor):
                    audio_np = audio_np.cpu().numpy()

                audio_chunk = (audio_np * 32767).astype(np.int16)
